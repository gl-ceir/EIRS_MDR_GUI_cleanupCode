...
//MDR
$('#dataTableDiv div:last').after('<p id="errorMsg" style="color: red;font-size: 15px;position: absolute;left: 32px;margin: 0;top: 97px;" class="left"></p>')

function checkDate(startDate, endDate) {
		console.log("start date " +$("#startDate").val() +" and end Date "+$("#endDate").val());
        ...
        var input1 = $("#startDate").val();
        var input2 = $("#endDate").val();
        ...
        if (searchParams == 'reqType=formPage') {
                ...
                if ((input1 <= input3) && (input2 <= input3)) {
                        if (input2 == input1) {
								...
								//$('#' + endDate.id).css('border-color', 'red');
								console.log("endDate.id-----"+JSON.stringify(endDate.id));
								console.log("endDate-----"+$.i18n(endDate));
								$('#errorMsg').text('');
                        ...
                        $('#errorMsg').text("End Date or Start Date should not be greater than today Date");
                        $('#submitFilter,#consignmentSubbmitButton,#filterFieldTable').addClass("eventNone");
                        $('#consignmentSubbmitButton').addClass("eventNone");
                }
        }
